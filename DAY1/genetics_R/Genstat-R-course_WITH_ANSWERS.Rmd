...
```{r ex-2-1}
indivs_na_summary <- naniar::miss_case_summary(data_in)
indivs_na_summary
```
...
```{r ex-2-2}
indivs_missing_10prcnt <- indivs_na_summary %>%
  filter(pct_miss > 10)
indivs_missing_10prcnt
data_in <- data_in[-indivs_missing_10prcnt$case, ]
```
...
```{r ex-3-1}
extra_indivs <- read_table(here("DAY1", "genetics_R", "data_c_intro_R.dat"))
extra_indivs
data_in

data_in <- data_in %>%
  bind_rows(extra_indivs)
data_in
...
write_delim(
  data_in,
  path = here("DAY1", "genetics_R", "data_prep_R.dat"),
  delim = " ",
  col_names = TRUE
)
...
# or:
length(table(ped_all$fam))
```

4.4 How many of the individuals were affected?

```{r ex-4-4}
table(ped_all$fam$affected)
```
